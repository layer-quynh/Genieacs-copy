...
function saveDevice(deviceId, deviceData, isNew, sessionTimestamp, callback) {
  let time;
  // $set: replace a value of a field with specified value
  // $unset: delete a particular field
  // $addToSet: add a nonduplicate field
  // $pull: delete a field
  ...
  if(isNew) {
    ...
    devicesCollection.update(
      ...
      (err, result) => {
        if (!err && result.result.n !== 1) {
          return void callback(
            new Error(`Device ${idDevice} not found in database`)
          );
        }
  ...
  devicesCollection.createIndex( { "_informedOn": 1 }, { expireAfterSeconds: 86400 } )

...
function endSession(sessionContext, callback) {
  ...
  let counter = 3;
  // sessionContext.timestamp = +sessionContext.timestamp;
...
function processRequest(sessionContext, rpc) {
  if (rpc.cpeRequest) {
 // console.log(rpc.cpeRequest.name);
...
//deviceInformation.getAllInformationDbDevices();
  ...
  stream.on("end", () => {
    ...
    function parsedRpc(sessionContext, rpc, parseWarnings) {
     /* console.log(deviceInformation.getAllInformationDevices({
        sessionContext: sessionContext,
        message: "CPE request",
        rpc: rpc
      }));*/
 
     /* console.log(deviceInformation.getOneInformationDevice({
        sessionContext: sessionContext,
        message: "CPE request",
        rpc: rpc
